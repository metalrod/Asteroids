		...
		while (window.pollEvent(event))
		{
			if (event.type == sf::Event::KeyPressed)
			{
				...
				{					
					...
					window.create(fullscreen ? sf::VideoMode(1920,1080) : sf::VideoMode(1280, 720), "Asteroid Game by SAF", fullscreen ? sf::Style::Fullscreen : sf::Style::Titlebar);
				}
               
            // NEUER CODE - VERÄNDERUNG - ESC
                // NEUER CODE - VERÄNDERUNG - ESC
			...
			if (!game.isGameOver())
			{
				...
				if (game.getProceed())
				{
					for (int i = 0; i < 5 + game.getLevel(); i++)
					{
						LargeAsteroid* newAsteroid = new LargeAsteroid(sf::Vector2f(randomLocationX(randomGenerator), randomLocationY(randomGenerator)));
                        // NEW CODE HERE
                        // Damit kann man die Texture noch nachträglich verändern!
                        // newAsteroid->reload("Sprites/PNG/Meteors/meteorGrey_med2.png");
                        // NEW CODE HERE
						newAsteroid->SetVelocity(randomSpeed(randomGenerator));
						game.AddObject(newAsteroid);
