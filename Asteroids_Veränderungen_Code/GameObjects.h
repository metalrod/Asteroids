...
public:
    ...
    // NEW CODE HERE
    virtual void reload(std::string texturePath);
    // NEW CODE HERE
...
private:
    // NEW CODE HERE
    // DAMIT das SChiff beim Respawn nicht schießt
	bool m_firing = false;
    // NEW CODE HERE
	float m_cooldown;
	float m_timer = 0.0f;
...
public:
    Bonus (std::string texturePath, const sf::Vector2f& pos);
    virtual void Update(sf::RenderWindow * window, float dt);
    virtual void Draw(sf::RenderWindow* window);
